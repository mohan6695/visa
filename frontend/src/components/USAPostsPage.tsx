...
export function USAPostsPage() {
  ...
  const fetchPosts = useCallback(async (query: string = '') => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/v1/sidebar/search?q=${encodeURIComponent(query)}&group_id=${USA_GROUP_ID}&limit=12`
      )
  ...
  const getAuthorInitials = (authorId: string | null) => {
    if (!authorId) return 'AN'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-sm text-gray-600">
              {loading ? (
                ...
                <span>{posts.length} posts {latency && <span className="text-green-600 ml-2">({latency.toFixed(0)}ms)</span>}</span>
              )}
            </div>
          </div>
                  ...
                  <div key={post.id} className="bg-gradient-to-r from-blue-50 to-white rounded-lg border border-blue-200 p-6">
                    <div className="flex items-start space-x-4">
                      <div className="flex flex-col items-center">
                        <button className="text-blue-600 hover:text-green-600"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 15l7-7 7 7"></path></svg></button>
                        <span className="text-lg font-semibold text-blue-700">{post.score}</span>
                        <button className="text-blue-600 hover:text-red-600"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg></button>
                      </div>
                      <div className="flex-1">
                        ...
                        <button onClick={() => setSelectedPost(post)} className="text-blue-600 hover:text-blue-700 text-sm font-medium">Read more →</button>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <div className="mt-8">
                  ...
                  <div className="grid gap-4">
                    {posts.slice(3).map((post) => (
                      <div key={post.id} className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md cursor-pointer" onClick={() => setSelectedPost(post)}>
                        <div className="flex items-start space-x-4">
                          <div className="flex flex-col items-center">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-lg p-6 sticky top-4">
            ...
            {latency && <div className="mb-4 p-3 bg-green-50 rounded-lg"><p className="text-sm text-green-800">⚡ {latency.toFixed(0)}ms</p></div>}
